...
function setup() {
  ...
  player.shapeColor = color(230, 255, 230);
...
function draw() {
  ...
  if (clouds.collide(player, gameover)) {
  }
...
// maybe for landing page make if else || like have start button as if [in the draw function] and the rest of the gane as else???? 
  
function gameover(player, cloud) {
// for (let i=0; i < 0; i++);
background (20);
